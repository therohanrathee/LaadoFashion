...
for item in "${urls[@]}"; do
    ...
    while [ $success -eq 0 ]; do
        ...
        curl -s -L -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" "$url" -o "$path"
        
        # Check if file is less than 5KB (means it's probably JSON error)
        size=$(wc -c < "$path")
        if [ $size -lt 5000 ]; then
            echo "Failed (Rate limited or error). Sleeping 12s..."
            sleep 12
        else
            echo "Success!"
            success=1
