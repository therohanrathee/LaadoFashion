    ...
    <section className="relative w-full overflow-hidden bg-[#FAF8F5] pt-[80px] min-h-[calc(100vh-80px)] flex items-center">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 w-full py-10 lg:py-12 mt-4 lg:mt-0">
        <div className="flex flex-col-reverse lg:grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="text-left w-full mt-4 lg:mt-0">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="inline-flex items-center gap-2 bg-[#E91E63]/5 border border-[#E91E63]/10 rounded-full px-4 py-2 mb-6">
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }} className="text-5xl sm:text-6xl lg:text-[4rem] font-serif font-bold leading-[1.08] mb-6 text-[#1a1a1a]">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.6 }} className="flex flex-wrap gap-2 lg:gap-3 mb-6 lg:mb-8">
               <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg shadow-sm border border-gray-100">
                  ...
                  <span className="text-xs sm:text-sm font-semibold text-gray-700">Home Measurement</span>
               </div>
               <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg shadow-sm border border-gray-100">
                  <Scissors className="w-4 h-4 text-[#E91E63]" />
                  <span className="text-xs sm:text-sm font-semibold text-gray-700">Custom Stitched</span>
               </div>
               <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-lg shadow-sm border border-gray-100 hidden sm:flex">
                  <Truck className="w-4 h-4 text-[#C5A55A]" />
                  <span className="text-xs sm:text-sm font-semibold text-gray-700">7-Day Delivery</span>
               </div>
            </motion.div>

            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.7 }} className="text-base lg:text-lg text-gray-600 font-light max-w-lg mb-8 leading-relaxed">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.2 }} className="mt-10 lg:mt-12 flex gap-8">
              {[ { number: '650+', label: 'Happy Clients' }, { number: '4.9★', label: 'Google Rating' } ].map((stat, i) => (
                <div key={i}>
                  <div className="text-2xl lg:text-3xl font-serif font-bold text-[#1a1a1a]">{stat.number}</div>
                  <div className="text-[10px] lg:text-xs text-gray-500 uppercase tracking-wider mt-1">{stat.label}</div>
                </div>
              ))}
            </motion.div>
          </div>

          {/* Right Column: Hero Image (Now visible on mobile!) */}
          <motion.div 
            initial={{ opacity: 0, scale: 0.95 }} 
            animate={{ opacity: 1, scale: 1 }} 
            transition={{ duration: 1, delay: 0.5 }} 
            className="relative block w-full aspect-[4/3] sm:aspect-video lg:aspect-[4.2/5] max-w-full lg:max-w-[460px] xl:max-w-[500px] lg:mx-auto rounded-3xl lg:rounded-2xl overflow-hidden shadow-2xl lg:-translate-y-2 xl:-translate-y-4"
          >
            <Image 
            ...
            {/* Floating Tag (Adjusted for mobile) */}
            <div className="absolute bottom-4 left-4 right-4 lg:bottom-6 lg:left-6 lg:right-6">
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-3 lg:p-4 flex items-center gap-3 lg:gap-4 text-white transform hover:-translate-y-1 transition-transform duration-300">
                <div className="bg-[#C5A55A] w-10 h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center shrink-0 shadow-lg">
                  <Scissors className="w-4 h-4 lg:w-5 lg:h-5 text-white" />
                </div>
                <div>
                  <div className="font-serif text-base lg:text-lg font-medium mb-0.5 leading-tight">Master Craftsmanship</div>
                  <div className="text-[10px] lg:text-xs text-white/90 font-light leading-tight">Over a decade of bringing bespoke elegance to life.</div>
                </div>
              </div>
            </div>
          </motion.div>
